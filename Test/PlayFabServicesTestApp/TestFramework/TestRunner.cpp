...
{
    // Init tracing
    RETURN_IF_FAILED(PFHCSettingsSetTraceLevel(PFHCTraceLevel::Verbose));
...
bool TestRunner::Update()
{
    if (m_activeTestClass == m_testClasses.end())
    {
        ...
        AddLog(PFHCTraceLevel::Important, testSummary.c_str());
        return true;
    }

    if (!m_activeTestClassInitialized)
    {
        ...
        // TODO: this should probably be changed to asynchronous at some point - ClassSetUp often involves async API
        // calls and this just blocks until they complete currently
        AddLog(PFHCTraceLevel::Verbose, "[TestClass] Starting");
        (*m_activeTestClass)->ClassSetUp();

        m_activeTestClassInitialized = true;
    }
    ...
    if (m_activeTest == m_activeTestClassTests.end())
    {
        ...
        ++m_activeTestClass;
        m_activeTestClassInitialized = false;
    ...
    case TestActiveState::PENDING:
    {
        // Begin test
        test->StartTest();
        AddLog(PFHCTraceLevel::Verbose, "[Test] Starting: %s", test->TestName().c_str());
    ...
    case TestActiveState::COMPLETE:
    {
        ...
        AddLog(PFHCTraceLevel::Important, "[Test] Completed: %s %s", test->TestName().c_str(), testFinishStateText.c_str());
...
void TestRunner::PFHCTraceCallback(_In_z_ const char* areaName, _In_ PFHCTraceLevel level, _In_ uint64_t threadId, _In_ uint64_t timestamp, _In_z_ const char* message)
{
