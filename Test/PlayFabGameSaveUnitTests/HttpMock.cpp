...
HttpMock::HttpMock(_In_ const char* method, _In_ const char* url) noexcept
{
...
void HttpMock::SetResponseBody(const PlayFab::JsonValue& responseBodyJson) const noexcept
{
...
) noexcept
{
    ...
    // avoid race between destruction and callback invocation
    std::lock_guard<std::mutex> lock{ mock->m_mutex };
    ++mock->m_hitCount;
    if (mock->m_callback)
    {
