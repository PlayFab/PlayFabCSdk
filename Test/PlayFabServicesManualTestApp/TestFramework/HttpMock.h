...
public:
    ...
    HttpMock(_In_ const char* httpMethod, _In_ const char* url, uint32_t statusCode) noexcept;
...
private:
    static void CALLBACK HCMockMatchedCallback(
        _In_ PFHCMockCallHandle mockHandle,
    ...
    std::mutex m_mutex;
    PFHCMockCallHandle m_handle{ nullptr };
    Callback m_callback;
