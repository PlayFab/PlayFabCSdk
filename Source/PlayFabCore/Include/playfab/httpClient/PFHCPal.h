...
#include <playfab/httpClient/PFHCConfig.h>
...
typedef uint32_t PFHCMemoryType;
typedef struct HC_WEBSOCKET_OBSERVER* PFHCWebsocketHandle;
typedef struct HC_CALL* PFHCCallHandle;
typedef struct HC_MOCK_CALL* PFHCMockCallHandle;
typedef struct HC_PERFORM_ENV* PFHCPerformEnv;
...
enum class PFHCWebSocketCloseStatus : uint32_t
{
    Normal = 1000,
    GoingAway = 1001,
    ProtocolError = 1002,
